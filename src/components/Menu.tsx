    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-8">
        <div className="text-center mb-8">
          ...
          <p className="text-lg text-gray-600">Elegí tu ensalada favorita y personalizála a tu gusto</p>
        </div>

        {/* Grid de ensaladas */}
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
          {activeSalads.map((salad) => (
            <div key={salad.id} className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
              <img 
                src={salad.image} 
                alt={salad.name}
                ...
                <div className="mb-4">
                  ...
                  <div className="flex flex-wrap gap-1">
                    {salad.baseIngredients.map(ingredientId => {
                      const ingredient = ingredients.find(i => i.id === ingredientId);
                      return ingredient ? (
                        <span key={ingredientId} className="bg-green-100 text-green-800 text-xs px-2 py-1 rounded">
                          {ingredient.name}
                        </span>
                      ) : null;
                    })}
                    ...
                    <div>
                      ...
                      {item.extraIngredients.length > 0 && (
                        <p className="text-sm text-gray-600">
                          Extras: {item.extraIngredients.map(id => 
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b">
              <div className="flex justify-between items-center">
                <h2 className="text-2xl font-semibold">Personalizar {selectedSalad.name}</h2>
                <button
            ...
            <div className="p-6 space-y-6">
              {/* Ingredientes base */}
              <div>
                <h3 className="text-lg font-semibold mb-4">Ingredientes base</h3>
              ...
              {/* Ingredientes extra */}
              <div>
                <h3 className="text-lg font-semibold mb-4">Agregar ingredientes</h3>
                            ...
                            >
                              <div className="flex items-center justify-between">
                                ...
                                <div className="flex items-center gap-1">
                                  {ingredient.price > 0 && (
                                    <span className="text-sm">+{formatPrice(ingredient.price)}</span>
                                  )}
