...
export default function SaladsPage() {
  ...
  const [formData, setFormData] = useState({
    ...
    calories: "",
    carbohydrates: "",
    type: "Ensalada" as "Ensalada" | "Tarta",
  ...
  const handleEditOpen = (salad: Salad) => {
    ...
    setFormData({
      ...
      calories: salad.calories?.toString() ?? "",
      carbohydrates: salad.carbohydrates?.toString() ?? "",
      image: salad.image,
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    if (
      ...
      formData.base.length === 0
    ) {
      showSnackbar("Complete los campos obligatorios (nombre, precio, calorías, carbohidratos y al menos una base)", {
        variant: "error",
      })
      return
    }
  ...
  const handleEdit = async (e: React.FormEvent) => {
    ...
    formDataToSend.append("calories", Number.parseFloat(formData.calories).toString())
    formDataToSend.append("carbohydrates", Number.parseFloat(formData.carbohydrates).toString())
    formDataToSend.append("type", formData.type)
  ...
  const baseOptions: Option[] = ingredients.map((ing) => ({
    value: ing._id,
    label: ing.name,
  }))

  const allowedExtrasOptions: Option[] = ingredients.map((ing) => ({
    value: ing._id,
    label: ing.name,
  }))
                ...
                {salads.length === 0 ? (
                  ...
                  salads.map((salad) => (
                    <tr key={salad._id} className="transition-colors hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-sm text-gray-900">{salad.calories ?? 0}</td>
                      <td className="px-6 py-4 text-sm text-gray-900">{salad.carbohydrates ?? 0}</td>
                      <td className="px-6 py-4 text-sm text-gray-600">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onChange={(option) =>
                      setFormData((prev) => ({
                        ...prev,
                        type: (option ? option.value : "Ensalada") as "Ensalada" | "Tarta",
                      }))
                    }
